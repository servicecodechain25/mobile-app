...
import { listSoldRecords } from '@/lib/db';

// GET - Export sold records to CSV
export async function GET(request) {
  try {
    ...
    records.forEach(record => {
      const purchaseAmount = parseFloat(record.purchase_amount) || 0;
      ...
      const row = [
        ...
        `"${record.purchase || ''}"`,
        purchaseAmount || '',
        record.purchase_date ? new Date(record.purchase_date).toLocaleDateString('en-IN') : '',
        `"${record.sold_name || ''}"`,
        soldAmount || '',
    ...
    const filename = `sold-records-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    console.error('Error exporting sold records:', error);
